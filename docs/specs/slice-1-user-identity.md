...
**Decision:** Netlify Blobs store `mtf-users`, one blob per user (`user:{id}`) plus `email:{normalized}` uniqueness keys. Classic Netlify Functions (`exports.handler`) must call `connectLambda(event)` before `getStore(name)` — do not pass Lambda `context` into `getStore`, and do not request `consistency: "strong"` (connectLambda does not set `uncachedEdgeURL`). Use `@netlify/blobs` 10.7+ `setJSON(..., { onlyIfNew: true })` (Node 16/18/20 compatible; v11 requires Node 22) and treat `modified: false` as taken. Tests inject an in-memory adapter. Do not silently fall back to memory in production.
