...
- Atmosphere: plate-rim / candle SVG only. No castle, no 🏰, no Disney marks
- Headline temperature: **“The table is being watched.”**

## Copy (fix the mock)

Keep the headline. Drop hotel/ops jargon from the mock: “The Method,” “bespoke,” “battery remain undisturbed,” “alert semantics,” pre-filled demo password.
...
3. Hero: “The table is being watched.” Subhead: newly opens, not every open time. Primary Sign in → `#signin`. Secondary “See how it works” → `#how`.
4. Landing IA: sticky header, hero, `#how` (three locked steps), trust strip (Watches · SMS · Every 10 min · Your phone only), `#proof` (California Grill watch card **and** the shared SMS with STOP/HELP), `#faq` (six required questions), `#signin`, footer.
5. FAQ answers stay honest: new openings only; signed-in profile’s phone; not official Disney; every 10 minutes; SMS consent at Create Watch; no table guarantee.
6. Sign-in uses existing IDs only: `#login-overlay`, `#login-profile`, `#login-pwd`, `#login-btn`, `#login-error`. Do not use mock IDs (`#profile-select`, `#password-input`, `#signin-form` as auth). Password starts empty. Real `attemptLogin` + `/_api/status`. Do not navigate to `app.html`.
