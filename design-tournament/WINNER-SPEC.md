...
4. Landing IA: sticky header, hero, `#how` (three steps: create a precise watch; we scan frequently; SMS only when a matching time newly opens), trust strip (Watches · SMS · Frequent scans · Your phone only), `#proof` (California Grill watch card **and** the shared SMS with STOP/HELP), `#faq` (six required questions), `#signin`, footer.
