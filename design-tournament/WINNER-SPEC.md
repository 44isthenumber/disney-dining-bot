...
Replaces the locked headline and subhead above. Headline: **"Sold out? We'll text you the moment a table opens."** Subhead explains the mechanism and states that booking happens on Disney's site on the guest's own account. Pricing leaves the subhead; it lives in a note under the CTA and in `#pricing`. Desktop hero is two columns: copy and a two-field guest starter (restaurant + dates + CTA, with quick-start chips for hard-to-get tables) on the left, the wand landing on the sample text on the right. Party, meal, time window, phone, email, and text consent appear once a restaurant and a date are chosen (`#create-watch.is-collapsed`); the consent gate and Stripe handoff are unchanged. CTA copy: "Start my watch · from $4.99" / "Start my watch · $4.99" (never "Continue to payment"). The signed-in app form is unaffected. An ink-background hero was tried and rejected the same day; the hero stays paper.
