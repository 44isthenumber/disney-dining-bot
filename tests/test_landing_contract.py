...
class LandingContractTest(unittest.TestCase):
    ...
    def test_production_auth_ids_not_mock_ids(self):
        ...
        self.assertNotIn("wordmark-wand-accent", INDEX)
        self.assertNotIn("wordmark-magic", INDEX)
        self.assertIn('class="wordmark-mark"', overlay)
        self.assertIn('class="wordmark-mark"', rest)
        self.assertIn("@media (max-width: 640px)", INDEX)
        self.assertIn(".landing-wordmark span.wordmark-name { display: none; }", INDEX)
        self.assertNotIn(".landing-wordmark .wordmark-mark { display: none; }", INDEX)
        self.assertGreaterEqual(INDEX.count('class="wordmark-mark"'), 2)
        self.assertIn('width="88"', INDEX)
        self.assertIn('height="62"', INDEX)
        self.assertIn('viewBox="0 0 180 126"', INDEX)
        self.assertIn('stroke-width="9.5"', INDEX)
        self.assertIn(".wordmark-mark { width: 72px; height: 50px; }", INDEX)
    ...
    def test_hero_wand_is_the_only_fireworks(self):
        ...
        self.assertNotIn("animation:", INDEX.split(".wordmark-mark {", 1)[1][:500])
