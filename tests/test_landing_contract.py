...
class LandingContractTest(unittest.TestCase):
    ...
    def test_production_auth_ids_not_mock_ids(self):
        ...
        self.assertIn("@media (min-width: 641px)", INDEX)
        self.assertIn(".landing-wordmark .wordmark-mark { display: none; }", INDEX)
        self.assertIn("@media (max-width: 640px)", INDEX)
        self.assertIn(".landing-wordmark span.wordmark-name { display: none; }", INDEX)
        self.assertIn("wordmark-magic", INDEX)
        self.assertGreaterEqual(INDEX.count('class="wordmark-wand-accent"'), 2)
        self.assertIn('viewBox="0 0 24 36"', INDEX)
        self.assertIn('stroke-width="1.85"', INDEX)
